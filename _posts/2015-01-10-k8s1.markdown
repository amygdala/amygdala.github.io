...
This post describes how to run a persistent installation of [Wordpress](https://wordpress.org/) on [Kubernetes](https://github.com/GoogleCloudPlatform/kubernetes).
...
Some of the post details, such as the Persistent Disk setup, require that Kubernetes is running on [Google Compute Engine](https://cloud.google.com/compute/).
...
Next, grab the Kubernetes [release binary](https://github.com/GoogleCloudPlatform/kubernetes/releases).  (This example was tested with release 0.8.1).

Then, start up a [Kubernetes](https://github.com/GoogleCloudPlatform/kubernetes) [cluster](...) as [described here](https://github.com/GoogleCloudPlatform/kubernetes/blob/master/docs/getting-started-guides/gce.md).
...
Copy and then edit this [mysql.yaml](https://gist.github.com/amygdala/88a8740e3946ba55125b) pod definition to use the database password you specify.  `mysql.yaml` looks like this:
...
{% highlight bash %}
$ <kubernetes>/cluster/$ <kubernetes>/cluster/kubectl.sh create -f mysql.yaml
{% endhighlight %}
...
[** ssh in, run `sudo -s`, run `docker ps`, run `docker logs <container_id>` **]
...
We will specifically name the service `mysql`.  This will let us leverage the support for [Docker-links-compatible](https://github.com/GoogleCloudPlatform/kubernetes/blob/master/docs/services.md#how-do-they-work) serviceenvironment variables when we up the wordpress pod. The wordpress Docker image expects to be linked to a mysql container named `mysql`, as you can see in the "How to use this image" section on the wordpress docker hub [page](https://registry.hub.docker.com/_/wordpress/).
...
Copy the [mysql-service.yaml](https://gist.github.com/amygdala/9f88e2ea9c37d26a8a68) file, which looks like this:
...
Copy this pod config file: [wordpress.yaml](https://gist.github.com/amygdala/ccf107f940054ae5d740) and edit the database password to be the same as you used in `mysql.yaml`. Note that this config file also defines a volume, this one using the `wordpress-disk` persistent disk that you created.
...
Once the wordpress pod is running, start its service.  Copy
[wordpress-service.yaml](https://gist.github.com/amygdala/72128b4624a7c9317a45).
...
{% highlight bash %}
$ <kubernetes>/cluster/kubectl.sh create -f frontend-service.yaml
{% endhighlight %}
...
Use the external IP that you obtained above, and visit it on port 3000:
